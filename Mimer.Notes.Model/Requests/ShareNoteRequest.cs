...
namespace Mimer.Notes.Model.Requests {
	public class ShareNoteRequest : ISignable, IRequestObject, INonRepeatableRequest {
		private JsonObject _json;

		public ShareNoteRequest() {
			_json = new JsonObject();
			TimeStamp = DateTime.UtcNow;
			RequestId = Guid.NewGuid();
		}

		public ShareNoteRequest(JsonObject json) {
			_json = json;
		}
		...
		public string Recipient {
			get {
				return _json.String("recipient");
			}
			set {
				_json.String("recipient", value);
			}
		}
